...
class AdminController extends Controller {
    ...
    protected function _initialize(){
        ...
        // 是否是管理员
        /*
         * 管理员在权限中可以获得auth_gourp
         *
         * */
        if(!isset($_SESSION['bus_admin']['place'])){
            if(empty($_SESSION['bus_admin']['place']['time'])){
                $place = M('ucenter_member')
                    ->alias(' a ')
                    ->field(' a.place,b.* ')
                    ->join(" left join bus_place as b on a.place = b.id ")
                    ->where( ' a.id='.UID)
                    ->find();
                empty($place['title'])?$place['title']='力久共享班车总后台':$place['title'];
        ...
        if(UID>=1){
            $Math = M('auth_group_access')->where(' uid= '.UID)->find();
            session('group_id',$Math['group_id']);
        ...
        $rule =$_SESSION['bus_admin']['rules']['rules'];
        $rules = explode(',',$rule);
        if($_SESSION['bus_admin']['place']['place']>0){
        ...
        $pace = $_SESSION['bus_admin']['place'];
        $this->assign('place',$pace);
    }
     ...
     * @author 朱亚杰  <zhuyajie@topthink.net>
     */
    protected function forbid ( $model , $where = array() , $msg = array( 'success'=>'状态调整成功！', 'error'=>'状态禁用失败！')){
        $data    =  array('status' => 0);
     ...
     * @author 朱亚杰  <zhuyajie@topthink.net>
     */
    protected function resume (  $model , $where = array() , $msg = array( 'success'=>'状态调整成功！', 'error'=>'状态恢复失败！')){
        $data    =  array('status' => 1);
        $this->editRow(   $model , $data, $where, $msg);
    }
    ...
    final public function getMenus($controller=CONTROLLER_NAME){
        ...
        $groupid = $_SESSION['bus_admin']['group_id'];
        $Math_assoc = M("auth_group")->where(" id =".$groupid)->field('rules')->find();
        session('rules',$Math_assoc);
        $menus  =   session('ADMIN_MENU_LIST.'.$controller);
        if(empty($menus)){
            if(UID >= 1){
                $where = " id in (".$Math_assoc['rules'].") and pid = 0 and hide = 0 and is_dev = 0";
            }else if(IS_ROOT){
                // 获取主菜单
                $where['pid']   =   0;
                $where['hide']  =   0;
