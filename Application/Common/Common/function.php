...
function checkIdCard($identitycardno){

    // 只能是18位
    if(strlen($identitycardno)!=18){
        return false;
    }

    // 取出本体码
    $idcard_base = substr($identitycardno, 0, 17);

    // 取出校验码
    $verify_code = substr($identitycardno, 17, 1);
 ...
 * ----------以下为返回信息----------
 * return  $code     int       状态
 * return $message  string    提示信息
 * return $data     array     详细数据
 */
#data || model,where show 其中一个方法存在即可#
#data 关闭 请在方法开头 发送 false#
function json_api($data=array(),$model='',$where=array('id'=>0),$show=1,$type){
    $model == 'session'?$type='session':$model=='cookie'?$type='cookie':'no';

    if(empty($data)||$data==false){
        $m = M($model);
        if($model == 'home_member'){
            if(isset($_SESSION['bus_home']['user_auth'])){
                $show==1? $data=$m->where($where)->find() :$data=$m->where($where)->select();
            }else{
                $code = 305;
            }
        }else{
            ...
            $show==1? $data=$m->where($where)->find() :$data=$m->where($where)->select();
            empty($data)?$code = 400:$code = 200;
        }
    }else{
        $data==false?$code = 400:empty($data)?$code = 400:$code = 200;
    }
   empty($type)?
       'no':empty($data)?
       'no':$type=='session'?
           session($model,$data):
           $type=='cookie'?cookie($model,$data):
   'no';
