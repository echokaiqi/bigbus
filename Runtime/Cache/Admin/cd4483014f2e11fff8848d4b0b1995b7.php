...
<head>
    ...
    <link href="/bus/Public/favicon.ico" type="image/x-icon" rel="shortcut icon">
...
<body>
    ...
    <tbody>
		<?php if(!empty($data)): if(is_array($data)): $i = 0; $__LIST__ = $data;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($i % 2 );++$i;?><tr>
			...
			<td><a href="<?php echo U('Bus/detail?pid='.$vo['id']);?>"><?php echo ($vo["title"]); ?></a></td>
			<td><?php echo ($vo["number"]); ?></td>
			<td><?php echo ($vo["nickname"]); ?></td>
			<td><span><?php echo ($vo["time"]); ?></span></td>
			<?php if($vo["status"] == 0 ): ?><td style="color:green;">允许运行</td>
			...
			<td>
				<a href="<?php echo U('Bus/get_oper?method=forbidrank&id='.$vo['id']);?>" class="confirm ajax-get">开启</a>
				<a href="<?php echo U('Bus/get_oper?method=resumerank&id='.$vo['id']);?>" class="confirm ajax-get">关闭</a>
				<a href="<?php echo U('Bus/get_oper?method=deletsrank&id='.$vo['id']);?>" class="confirm ajax-get" style="color:red;">删除</a>
            </td>
    ...
    //导航高亮
    highlight_subnav('<?php echo U('Bus/index');?>');
	</script>
