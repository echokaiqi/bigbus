...
<body>
    ...
    <tbody>
		<?php if(!empty($data)): if(is_array($data)): $i = 0; $__LIST__ = $data;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($i % 2 );++$i;?><tr>
			...
			<td><a href="<?php echo U('User/behavior?uid='.$vo['id']);?>"><?php echo ($vo["username"]); ?></a></td>
		...
		<?php else: ?>
		<td colspan="9" class="text-center"> oh! 目前还没有会员! 尽快推广吧~ </td><?php endif; ?>
	</tbody>
    </table>
	</div>
    <div class="pages">
    ...
    //导航高亮
    highlight_subnav('<?php echo U('User/detail');?>');
	</script>
