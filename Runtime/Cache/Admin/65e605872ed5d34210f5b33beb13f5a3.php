...
<body>
    ...
    <tbody>
		<?php if(!empty($_list)): if(is_array($_list)): $i = 0; $__LIST__ = $_list;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($i % 2 );++$i;?><tr>
			...
			<?php if($vo["uid"] == $uid): ?><td style="color:green;">无法操作自己</td>
				<?php elseif( $vo["uid"] == 1 ): ?>
				...
				<td><?php if(($vo["status"]) == "1"): ?><a href="<?php echo U('User/changeStatus?method=forbidUser&id='.$vo['uid']);?>" class="ajax-get">禁用</a>
					...
					<?php if( $grop == 2): else: ?>
						<a href="<?php echo U('AuthManager/group?uid='.$vo['uid']);?>" class="authorize">授权</a><?php endif; ?>

					<a href="<?php echo U('User/userSend?uid='.$vo['uid']);?>" class="authorize">派遣</a>
