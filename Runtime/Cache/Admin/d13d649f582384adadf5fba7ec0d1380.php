...
<body>
    ...
    <tbody>
		<?php if(!empty($data)): if(is_array($data)): $i = 0; $__LIST__ = $data;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($i % 2 );++$i;?><tr>
			...
			<td><a href="<?php echo U('Bus/detail?rid='.$vo['id']);?>"><?php echo ($vo["title"]); ?></a></td>
			<td><?php echo ($vo["start"]); ?></td>
			<td><?php echo ($vo["end"]); ?></td>
			<td><span><?php echo ($vo["nickname"]); ?></span></td>
			<td><span><?php echo ($vo["time"]); ?></span></td>
			<td>
				<?php if($vo["bodytime"] < 20): ?><span style="color:green"><?php echo ($vo["bodytime"]); ?></span>
				<?php elseif($vo["bodytime"] < 50): ?>
				<span style="color:blueviolet"><?php echo ($vo["bodytime"]); ?></span>
				<?php else: ?>
				<span style="color:brown"><?php echo ($vo["bodytime"]); ?></span><?php endif; ?>
			</td>
			<?php if($vo["status"] == 0 ): ?><td style="color:green;">正在运行</td>
			<?php elseif($vo["status"] == 1 ): ?>
				<td style="color:blue;">等待运行</td>
			<?php else: ?>
				<td style="color:darkred;">停止运行</td><?php endif; ?>

			<td>
				<a href="<?php echo U('Route/get_oper?method=forbidroute&id='.$vo['id']);?>" class="confirm ajax-get">开放</a>
				<a href="<?php echo U('Route/get_oper?method=resumeroute&id='.$vo['id']);?>" class="confirm ajax-get">等待</a>
                <a href="<?php echo U('Route/get_oper?method=deleteRoute&id='.$vo['id']);?>" class="confirm ajax-get">关闭</a>
				<a href="<?php echo U('Route/get_oper?method=deletsRoute&id='.$vo['id']);?>" class="confirm ajax-get" style="color:red;">删除</a>
            </td>
		</tr><?php endforeach; endif; else: echo "" ;endif; ?>
		<?php else: ?>
		<td colspan="9" class="text-center"> Oh! 暂时还没有线路! </td><?php endif; ?>
	</tbody>
    </table>
	</div>
    <div class="pages">
	...
	<script src="/bus/Public/static/thinkbox/jquery.thinkbox.js"></script>
    ...
    //导航高亮
    highlight_subnav('<?php echo U('Route/index');?>');
	</script>
