...
<body>
    ...
    <tbody>
		<?php if(!empty($data)): if(is_array($data)): $i = 0; $__LIST__ = $data;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($i % 2 );++$i;?><tr>
			...
			<td><a href="<?php echo U('Driver/detail?drid='.$vo['id']);?>"><?php echo ($vo["nickname"]); ?></a></td>
			<?php if($vo["sex"] == 0 ): ?><td >男</td>
			<?php else: ?>
				<td >女</td><?php endif; ?>
			<td><?php echo ($vo["mobile"]); ?></td>
			<td><?php echo ($vo["age"]); ?></td>
			<td><?php echo ($vo["driv_ex"]); ?>
			<td><?php echo ($vo["identitycardno"]); ?></td>
			<td><?php echo ($vo["time"]); ?></td>
			<?php if($vo["level"] == 0): ?><td>普通会员</td>
			<?php elseif($vo["level"] == 1): ?>
				<td style="color:red;">VIP会员</td>
			<?php else: ?>
				<!--end...--><?php endif; ?>
			<td>
				<!--<a href="<?php echo U('Bus/get_oper?method=deletsrank&id='.$vo['id']);?>" class="confirm ajax-get"  >删除</a>-->
				<a href="<?php echo U('Driver/del?method=deletsdrive&id='.$vo['id']);?>" class="confirm ajax-get" >删除</a>
			</td>
		</tr><?php endforeach; endif; else: echo "" ;endif; ?>
		<?php else: ?>
		<td colspan="9" class="text-center"> oh! 目前还没有司机! 尽快招聘吧~ </td><?php endif; ?>
	</tbody>
    </table>
	</div>
    <div class="pages">
    ...
    //导航高亮
    highlight_subnav('<?php echo U('Driver/index');?>');
	</script>
